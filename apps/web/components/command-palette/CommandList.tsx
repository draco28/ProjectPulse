...
export function CommandList({ categories }: CommandListProps) {
  const { filteredCommands, selectedIndex, setSelectedIndex, executeCommand, searchQuery } =
    useCommandPalette();
  ...
  if (!filteredCommands.length) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-slate">
        ...
        {searchQuery && <p className="mt-1 text-xs">Try a different search term</p>}
      </div>
    );
  }
  ...
  return (
    <div ref={listRef} className="scrollbar-auto-hide max-h-96 overflow-y-auto" role="listbox">
        ...
        <div key={category.id} className={catIndex > 0 ? 'border-t border-[#1F1F1F] p-4' : 'p-4'}>
          ...
          <div className="mt-1 space-y-1">
            {category.commands.map((command) => {
              ...
              return (
                <div key={command.id} ref={isSelected ? selectedItemRef : null}>
                  <CommandItem
                    command={command}
