...
async function testUnifiedEmbedding() {
  ...
  try {
    ...
    if (result.embedding.length === 768) {
      ...
      console.log(`\nProvider Summary:`);
      console.log(
        `  - Primary: ${providers.ollama ? 'Ollama (nomic-embed-text, free)' : 'Not available'}`
      );
      console.log(
        `  - Fallback: ${providers.openai ? 'OpenAI (text-embedding-3-large, paid)' : 'Not configured'}`
      );
      console.log(
        `  - Active: ${result.provider === 'ollama' ? 'Ollama ✅' : 'OpenAI (fallback)'}`
      );
    } else {
