...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!session) {
      ...
      console.log('[POST /api/onboarding/documents/batch] Session 2 created', { sessionId: session.id });
    }
    ...
    if (existingDocs.length > 0) {
      console.error('[POST /api/onboarding/documents/batch] Duplicate filenames found:', existingDocs);
